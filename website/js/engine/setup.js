...
ontouchend = e => {
	for (let i=0; i<e.changedTouches.length; i++) {
		for (let j=Mouse.touches.length-1; j>=0; j--) {
			if (e.changedTouches[i].identifier == Mouse.touches[j].identifier) {
				Mouse.touches[j].down = false;
				// socket.emit('new path', JSON.stringify(Mouse.touches[j].path), isRubber);
				processPathDrawing(Mouse.touches[j]);
				if (Mouse.touches[j].start.x < 200 && Mouse.touches[j].start.y > CANVASHEIGHT/2) {
					drawUI = !drawUI;
				}
			}
		}
...
window.addEventListener('touchend', () => c.requestFullscreen());